...
class AFKMixin:
    ...
    def set_afk(self, user_id: int, guild_id: int, reason: str = "") -> None:
        ...
        try:
            ...
            logger.error_tree("DB: Set AFK Error", e, [
                ("ID", str(user_id)),
                ("Guild ID", str(guild_id)),
            ])
    ...
    def remove_afk(self, user_id: int, guild_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error_tree("DB: Remove AFK Error", e, [
                ("ID", str(user_id)),
                ("Guild ID", str(guild_id)),
            ])
            return None
    ...
    def get_afk(self, user_id: int, guild_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error_tree("DB: Get AFK Error", e, [
                ("ID", str(user_id)),
                ("Guild ID", str(guild_id)),
            ])
            return None
    ...
    def get_afk_users(self, user_ids: List[int], guild_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error_tree("DB: Get AFK Users Error", e, [
                ("User Count", str(len(user_ids))),
                ("Guild ID", str(guild_id)),
            ])
            return []
    ...
    def get_expired_afk_users(self, max_age_seconds: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error_tree("DB: Get Expired AFK Error", e, [
                ("Max Age", str(max_age_seconds)),
            ])
            return []
    ...
    def increment_afk_mentions(self, user_id: int, guild_id: int, pinger_id: int = None, pinger_name: str = None) -> None:
        ...
        try:
            ...
            logger.error_tree("DB: AFK Mention Track Error", e, [
                ("ID", str(user_id)),
                ("Pinger", pinger_name or "Unknown"),
            ])
    ...
    def get_and_clear_afk_mentions(self, user_id: int, guild_id: int) -> tuple[int, List[str]]:
        ...
        try:
            ...
            logger.error_tree("DB: Clear AFK Mentions Error", e, [
                ("ID", str(user_id)),
                ("Guild ID", str(guild_id)),
            ])
            return 0, []
